...
# except Exception:
#     pass  # secrets.toml nahi hai toh skip karo, .env se chalega
...
#             all_threads = []
#             async for thread in checkpointer.list(config={}, limit=500):
...
#     # ✅ KEY FIX: token ko is thread ke os.environ mein set karo
#     # _THREAD_POOL ka alag environ hota hai, isliye yahan set karna zaroori hai
#     if token:
...
#                         "role": "agent",
#                         "content": "⚠️ LinkedIn Access Token nahi diya! Sidebar mein token daalo phir dobara try karo."
#                     })
...
#                 if confirm_publish:
#                     # ✅ Token ko state mein inject karo before resuming
#                     current_state = await graph.aget_state(config)
#                     await graph.aupdate_state(
#                         config,
#                         {"linkedin_access_token": token},
...
# # ✅ Token input — session_state + os.environ dono mein set karo
...
#     os.environ["LINKEDIN_ACCESS_TOKEN"] = raw_token  # ✅ main thread environ mein bhi
